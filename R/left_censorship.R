...
test_left_censorship = function(
  ...
  if (sum(missing_loc) == 0) {
    arg = rlang::caller_arg(data_matrix)
    call = rlang::caller_env()
    cli::cli_inform(
      message = c(
        'i' = '{.arg {arg}} has no missing values, returning NULL'
      )
    )
    return(NULL)
  }
  ...
  split_counts = purrr::imap(split_indices, \(in_split, split_id) {
    ...
    # get sample medians
    sample_medians = calculate_matrix_medians(
      split_missing,
      use = "col",
      na.rm = TRUE
    )
    ...
    all_trials = (nrow(keep_miss_updown) * ncol(keep_miss_updown)) -
      sum(is.na(keep_miss_updown))
  ...
  return(list(values = split_counts, binomial_test = binom_res))
}
